...
import { cases, fixtureVerdict } from '../shared/cases';
...
describe('independent replay and measurement honesty', () => {
  it('keeps feeding later evidence to both providers after a violation', async () => {
    ...
    const streams = await Promise.all(([0, 1] as const).map(i => replayCase(sample, async input => {
      ...
      return { ...fixtureVerdict(sample.snapshots.find(s => s.text === input.text)!), provider: i === 0 ? 'jev' : 'llm', source: 'provider-replay', serviceMs: 1 };
  ...
  it('does not report fixture wall time as model service time', async () => {
    const test = cases[0];
    const rows = await replayCase(test, async () => fixtureVerdict(test.snapshots[0]), 'fixture', new AbortController().signal);
    expect(rows[0].checkStartedOffsetMs).toBeNull();
    expect(rows[0].verdictOffsetMs).toBeNull();
    expect(summarize(rows, 'fixture').input.count).toBe(0);
  });
